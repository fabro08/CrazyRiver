...
public class Grafo {
	...
	public Grafo(int cantidadN){
		...
		listaAristas=new Arista[nodosMaximos*nodosMaximos];
	 ...
	 */
	public void insertarNodo(Object datoN,double ejeX,double ejeY,boolean domo, boolean combustible,boolean nodoFinal){
        //verifica que el grafo no este lleno
        if (cantidadNodos <= nodosMaximos) {
            //Agrega el nodo al grafo
            listaNodos[cantidadNodos++]= new Nodo(datoN,ejeX,ejeY,domo,combustible,nodoFinal);
        }
        // si esta lleno
        else{
	 ...
	 */
    public void insertarArista(int inicio, int fin,int peso,double ejeX, double ejeY, boolean dirijida){
        ...
        if (inicio <= nodosMaximos && fin <= nodosMaximos && dirijida==false) {
        	...
        	listaAristas[cantidadAristas++]=new Arista(inicio,fin,peso,ejeX,ejeY,dirijida);
    ...
    public void imprimirAristas(){
        ...
        for (int i = 0; i < listaAristas.length; i++) {
        	if(listaAristas[i]!=null){
                //imprime las Aristas
        		System.out.println("("+listaAristas[i].getInicio()+" , "+listaAristas[i].getFin()+")");
        	}
        }
    }
